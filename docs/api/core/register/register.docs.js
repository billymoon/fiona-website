...
const Section = ({ seed }) => (
  ...
  >
    ...
    <p>
      The <code>Fiona.register</code>{" "}
      method accepts one argument which should be a function that takes a seeded
      instance, and returns either a value, or the seeded instance to allow
      chaining.
    </p>

    <p>
      In this example we will create a <code>person</code>{" "}
      extension that generates some data for a person, ensuring the gender is
      correct for each data item.
    </p>
