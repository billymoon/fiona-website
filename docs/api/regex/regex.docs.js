...
const Section = ({ seed }) => (
  ...
  >
    <p>
      ...
      <a href="http://npmjs.org/packages/randexp">randexp</a>{" "}
      library to expand expressions, seeded by the instance of Fiona.
    </p>
    ...
    <p>
      When recursing objects, strings and arrays, any regex that's found will
      be{" "}
    </p>
    ...
    ${Fiona(seed)
        .string
        `There is an army of ${/[01]{8} (ro|cy)bo(rg|t)s/} on their way!`}
    `}
    />
    ...
    <p>
      <b>n.b.</b>{" "}
      the regex method is only available on the main Fiona package and is not
      avaliable on the Fiona core package which focusses on being very light so
      does not include the external libraries required to expand regular
      expressions.
    </p>
  </ApiSection>
);
