...
const Section = ({ seed }) => (
  ...
  >
    <p>
      ...
      (how many items in array) and a callback that is called with{" "}
      <code>seeded</code> which should return the value of each array item.
    </p>
    ...
    `}
      output={`\n${
        JSON.stringify(
          Fiona(seed).array(
            10,
            (seeded) => seeded.distribution((i) => i ** 10).number(),
          ),
          null,
          2,
        )
